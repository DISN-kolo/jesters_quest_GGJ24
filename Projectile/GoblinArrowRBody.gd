...
func _physics_process(delta):
	...
	#print((pos_prev - global_position).length())
	if (pos_prev - global_position).length() < 6:
		slow_killer(0.2)
...
func slow_killer(timetokill):
	...
	tween.tween_property($GoblinArrowSprite, "modulate", Color(1, 1, 1, 0), timetokill)
